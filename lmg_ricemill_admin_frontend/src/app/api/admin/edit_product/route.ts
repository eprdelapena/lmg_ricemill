...
import { TParamsEditProduct, TUserSession } from "@/schema/main_schema";
...
export async function POST(req: Request) {
  try {
    ...
    const body: TParamsEditProduct = await req.json();
    const response = await client.mainEditProduct(body, config);
