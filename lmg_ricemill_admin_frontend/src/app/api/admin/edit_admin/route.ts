...
import { TParamsEditAdmin, TUserSession } from "@/schema/main_schema";
...
export async function POST(req: Request) {
  try {
    ...
    const body: TParamsEditAdmin = await req.json();
    const response = await client.editAdmin(body, config);
