...
import { TParamsGetMonthlyIncome, TUserSession } from "@/schema/main_schema";
...
export async function POST(req: Request) {
  try {
    ...
    const body: TParamsGetMonthlyIncome = await req.json();
    const response = await client.mainGetMonthlyIncome(body, config);
