...
import { TParamsPostOrderUser, TUserSession } from "@/schema/main_schema";
...
export async function POST(req: Request) {
  try {
    ...
    const body: TParamsPostOrderUser = await req.json();
    const response = await client.mainPostOrderUser(body, config);
