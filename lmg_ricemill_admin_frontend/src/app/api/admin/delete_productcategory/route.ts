...
import { TParamsDeleteProductCategory, TParamsPostInstallment, TParamsPostProductCategory, TUserSession } from "@/schema/main_schema";
...
export async function POST(req: Request) {
  try {
    ...
    const body: TParamsDeleteProductCategory = await req.json();

    const response = await client.mainDeleteProductCategory(body, config);
