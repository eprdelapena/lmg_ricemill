...
import { TParamsGetOrderUser, TUserSession } from "@/schema/main_schema";
...
export async function POST(req: Request) {
  try {
    ...
    const body: TParamsGetOrderUser = await req.json();
    const response = await client.mainGetOrderUser(body, config);
