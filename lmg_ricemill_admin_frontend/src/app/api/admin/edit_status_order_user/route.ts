...
import { TParamsEditStatusOrderUser, TUserSession } from "@/schema/main_schema";
...
export async function POST(req: Request) {
  try {
    ...
    const body: TParamsEditStatusOrderUser = await req.json();
    const response = await client.mainEditStatusOrderUser(body, config);
