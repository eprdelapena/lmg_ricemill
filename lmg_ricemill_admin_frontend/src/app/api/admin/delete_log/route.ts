...
import { TParamsDeleteLogTable, TParamsGetAdmin, TParamsGetLogTable, TUserSession } from "@/schema/main_schema";
...
export async function POST(req: Request) {
  try {
    ...
    const body: TParamsDeleteLogTable = await req.json();
    const response = await client.mainDeleteLog(body, config);
