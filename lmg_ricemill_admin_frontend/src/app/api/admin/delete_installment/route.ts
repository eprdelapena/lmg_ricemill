...
import { TParamsDeleteInstallment, TUserSession } from "@/schema/main_schema";
...
export async function POST(req: Request) {
  try {
    ...
    const body: TParamsDeleteInstallment = await req.json();
    const response = await client.mainDeleteInstallment(body, config);
