...
import { TParamsGetAdmin, TUserSession } from "@/schema/main_schema";
...
export async function POST(req: Request) {
  try {
    ...
    const body: TParamsGetAdmin = await req.json();
    const response = await client.mainGetAdmin(body, config);
