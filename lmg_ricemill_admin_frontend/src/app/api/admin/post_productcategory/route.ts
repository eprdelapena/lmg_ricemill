...
import { TParamsPostInstallment, TParamsPostProductCategory, TUserSession } from "@/schema/main_schema";
...
export async function POST(req: Request) {
  try {
    ...
    const body: TParamsPostProductCategory = await req.json();

    const response = await client.mainPostProductCategory(body, config);
